...
,mFFTrta(NULL)
,mCaptureSleep(1)
,mIsDualStreams(false)
{
	mPortStreamV19_Main = NULL;
	mPortStreamV19_Aux = NULL;
...
int AudioIO::doIODevicesCalibration()
{
	...
	if (errorCode == 0)
	{
		...
		if (GetCurrentIOConfiguration(mCaptureSampleRate, captureDevIdx, captureChannels, playbackDevIdx, playbackChannels))
		{
			...
			if(mIsDualStreams)
				errorCode = OpenDevicesDualStream(mCaptureSampleRate, captureDevIdx, captureChannels, playbackDevIdx, playbackChannels); 
			else
			...
			if (errorCode == 0)
			{
				...
				while (bIsStopped == false)
				{
					...
					//Test signal generation 
					if(mIsDualStreams)
						errorCode = Pa_WriteStream(mPortStreamV19_Aux, OutputBuffer, mCaptureFrameSize);
					else
						errorCode = Pa_WriteStream(mPortStreamV19_Main, OutputBuffer, mCaptureFrameSize);
					...
					errorCode = Pa_ReadStream(mPortStreamV19_Main, InputBuffer, mCaptureFrameSize);
					if (errorCode != 0)
					{
						reportEvent(1, errorCode, wxT("PortAudio Error"));
						break;
				...
				if( mIsDualStreams)
					errorCode = CloseDevicesDualStream();
				else
	...
	reportEvent(1, errorCode, wxT("Calibration Complete\nSettings Saved."), true);
	...
	int playbackChannels)
{
	...
	err = Pa_OpenStream(&mPortStreamV19_Main,
	...
	if (err == paNoError)
	{
		err = Pa_StartStream(mPortStreamV19_Main);
		if (err == paNoError)
		{
			return paNoError;
		}
		Pa_CloseStream(mPortStreamV19_Main); 
	}
...
AudioIO::CloseDevices()
{
	...
	Pa_StopStream(mPortStreamV19_Main);
	err = Pa_CloseStream(mPortStreamV19_Main);
...
//experimental
PaError
AudioIO::OpenDevicesDualStream(double sampleRate,
	int captureDeviceIdx,
	int captureChannels,
	int playbackDeviceIdx,
	int playbackChannels)
{
	...
	err = Pa_OpenStream(&mPortStreamV19_Aux,
		NULL,
		&playbackParameters,
		sampleRate,
		mCaptureFrameSize,
	...
	{
		err = Pa_StartStream(mPortStreamV19_Aux);
		if (err != paNoError)
		{
			Pa_CloseStream(mPortStreamV19_Aux);
			return err;
		}
	}
...
PaError
AudioIO::CloseDevicesDualStream()
{
	PaError err = paNoError;
	...
	Pa_StopStream(mPortStreamV19_Aux);
	err = Pa_CloseStream(mPortStreamV19_Aux);
