...
typedef struct StreamConfiguration {
	double sampleRate;
	int deviceIdx;
	int devChannels;
	size_t frameSize;
...
public:
   ...
   StreamConfiguration getCurrentInputConfig();
   StreamConfiguration getCurrentOutputConfig();
 ...
 protected:
   ...
   int getInputDevIndex(const wxString &hostName, const wxString &devName);
   int getOutputDevIndex(const wxString &hostName, const wxString &devName);
