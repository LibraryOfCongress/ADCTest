...
public:
    ...
    mpInfoCoords(wxRect rect, const wxBrush* brush = wxTRANSPARENT_BRUSH, float fstep = 1);
    ...
    void SetLogarithmic( bool val ) { m_logarithmic = val; }
...
/** Set label for X axis in log frequency mode*/
#define mpX_LOGF 0x05
...
public:
    ...
    mpScaleX(wxString name = wxT("X"), int flags = mpALIGN_CENTER, bool ticks = true, unsigned int type = mpX_NORMAL, float fstep = 1);
...
class WXDLLIMPEXP_MATHPLOT mpScaleXSpectrum : public mpLayer
{
public:
    /** Full constructor.
		@param name Label to plot by the ruler
		...
		@param ticks Select ticks or grid. Give TRUE (default) for drawing axis ticks, FALSE for drawing the grid.
		@param type mpX_NORMAL for normal labels, mpX_TIME for time axis in hours, minutes, seconds. */
    mpScaleXSpectrum(wxString name = wxT("X"), int flags = mpALIGN_CENTER, bool ticks = true, unsigned int type = mpX_NORMAL, float fstep = 1);
    ...
    /** Set X axis to logarithmic scale
        @param ticks TRUE to plot axis ticks, FALSE to plot grid. */
    void SetLogarithmic(bool log) { m_logarithmic = log; };
...
protected:
    ...
    std::vector<FAxisXPoint> mFLogPoints;
    std::vector<FAxisXPoint> mFLinPoints;

    DECLARE_DYNAMIC_CLASS(mpScaleXSpectrum)
};
...
/** Canvas for plotting mpLayer implementations.
    ...
    Since wxMathPlot version 0.03, the mpWindow incorporates the following features:
        - floatBuffering (Default=disabled): Can be set with EnableDoubleBuffer
...
//     wxCoord y2p(float y, bool drawOutside = true); // { return (wxCoord) ( (m_posY-y) * m_scaleY); }
    ...
    /** Set X axis to logarithmic scale
    @param ticks TRUE to plot log axis , FALSE to linear. */
    void SetLogarithmic(bool log) { m_logarithmic = log; };

    /** Enable/disable x axis select mode
    */
    void EnableXSelectMode( bool enabled ) { m_enableXSelectMode = enabled; }
    float getSelXStart();
    float getSelXEnd();

    /** Enable/disable drawing mode
    */
    void EnableDrawingMode( bool enabled ) { m_enableDrawingMode = enabled; }
    std::vector<drawnPoint> ReturnDrawnPoints();
...
protected:
    ...
    bool        m_enableDoubleBuffer;  //!< For float buffering
    ...
    bool        m_enableDrawingMode;
    bool        m_enableXSelectMode;
    bool        m_logarithmic;
    float      m_XSelectStart;
    float      m_XSelectEnd;
    float      m_XSelectWidth;
    std::vector<drawnPoint> mDrawnPoints;
    std::vector<float> mSelectedXPoints;
...
/** An arbitrary polygon, descendant of mpMovableObject.
  ...
  *   passing the appropriate parameters to "setPoints". To draw a point-cloud, call "SetContinuity(false)".
  */
class WXDLLIMPEXP_MATHPLOT mpCursor : public mpMovableObject
{
public:
    /** Default constructor.
      */
    mpCursor( const wxString & layerName = wxT("") )
    {
        m_continuous = true;
        m_name = layerName;
    }
